...
The system detects the start of walking using initial lower-body motion and tracks elapsed time while the subject walks a predefined distance. Distance completion is estimated using pose-based movement proxies and stopping conditions are triggered when the target range is reached.
...
A multi-phase finite-state machine tracks four sequential phases: ascent, descent, ascent, descent. Each phase is validated using foot position relative to detected stair reference lines.
...
and mapped to clinical descriptors such as normal performance, mild impairment, moderate impairment, or severe impairment.
